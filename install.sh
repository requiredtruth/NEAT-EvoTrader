...
verify() { "$PY" -c 'import numpy, matplotlib; import numba; print("dependencies: OK")'; (cd "$ROOT" && "$PY" -m compileall -q main.py LIB tests && "$PY" -m unittest discover -s tests -v); }
demo() { mkdir -p "$MPLCONFIGDIR"; (cd "$ROOT" && "$PY" main.py --generations 2 --population 8 --seed 7 --run-dir "${TMPDIR:-/tmp}/neat-evotrader-demo"); }
doctor() { need_python; [ -x "$PY" ] || die "environment missing; run ./doit.sh install"; verify; say "doctor: PASS"; }
backup() { out=${2:-"$ROOT/neat-evotrader-backup-$(date +%Y%m%dT%H%M%S).tar.gz"}; tar -czf "$out" -C "$ROOT" CONFIG DATA RUNS BEST; say "backup=$out"; }
restore() { archive=${2:-}; [ -n "$archive" ] && [ -f "$archive" ] || die "usage: ./doit.sh restore BACKUP.tar.gz"; tar -tzf "$archive" | awk '/(^|\/)\.\.($|\/)|^\// {bad=1} END {exit bad}' || die "unsafe backup paths"; tar -xzf "$archive" -C "$ROOT"; say "restore complete"; }
...
case "$COMMAND" in
  ...
  *) die "usage: ./doit.sh {install|start|stop|restart|status|repair|migrate|backup [FILE]|restore FILE|logs|doctor|uninstall|test}" ;;
esac
